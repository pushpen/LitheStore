...
define('THEMEVAN_THEME_NAME', 'LitheStore');
...
function require_license_key_notice() {
	 ...
	 $user_id = $current_user->ID;
      if ( ! get_user_meta($user_id, 'ignore_license_key_notice') ) {
        ...
        printf(__('Enter your license key of '.THEMEVAN_THEME_NAME.' theme to activate the theme updater, <a href="'.home_url().'/wp-admin/options-general.php#licensekey">click here</a> | <a href="%1$s">Hide Notice</a>','lithestore'), '?hide_license_key_notice=0');
        echo "</p></div>";
	    }
}
...
class new_general_setting {
    ...
    function register_fields() {
        ...
        add_settings_field('fav_color', '<a id="licensekey"></a><label for="license_key">'.THEMEVAN_THEME_NAME.' License Key</label>' , array(&$this, 'fields_html') , 'general' );
    }
    function fields_html() {
        $value = get_option( 'license_key', '' );
        echo '<input type="text" id="license_key" name="license_key" value="' . $value . '" /><p>'.esc_html__('You can get the license key from your ThemeVan account. If you can\'t find it, please <a href="http://themevan.com/contact" target="_blank">contact us</a>.</p>','lithestore');
    }
}
